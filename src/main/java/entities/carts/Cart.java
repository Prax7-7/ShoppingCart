...
public class Cart implements ICart {
    ...
    public boolean addItem(String item, long quantity){
        if(!CartHelper.isItemFromInventory(item)){
            ...
            System.out.println(String.format("Quantity cannot be zero, hence item %s not added to cart",item));
            return false;
        }

        else if(quantity < 0L){
            System.out.println(String.format("Quantity cannot be negative, hence item %s not added to cart",item));
            return false;
        }

        else if(cartItemsMap.containsKey(item)){
            long initQurantity, updatedQuantity;
                  initQurantity  = cartItemsMap.get(item);
                  updatedQuantity = initQurantity+quantity;

            System.out.println(String.format("Item %s is already present in the cart, hence updated the quantity from %s to" +
                    " %s",item, initQurantity, updatedQuantity));
